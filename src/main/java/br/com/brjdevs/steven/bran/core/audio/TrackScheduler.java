...
public class TrackScheduler {
	...
	public boolean request(TrackContext track, boolean silent) {
		if (track.getInfo().length > AudioUtils.MAX_SONG_LENGTH) {
			track.getContext().sendMessage("Could not add `" + track.getInfo().title + "` to the queue. Reason: `This song it'currentArgs too long, the maximum supported length is 3 hours!").queue();
