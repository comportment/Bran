	...
	public void playlistLoaded(AudioPlaylist playlist) {
		...
		if (playlist.getSelectedTrack() != null) {
			...
			if (playlist.isSearchResult()) {
				...
				channel.sendMessage(Util.getUser(user) + ", results found by `" + trackUrl.substring(9).trim() + "`:\n" + String.join("\n", tracks.stream().map(track -> "`[" + (tracks.indexOf(track) + 1) + "]` " + track.getTrack().getInfo().title + " (`" + AudioUtils.format(track.getTrack().getInfo().length) + "`)").collect(Collectors.toList()))).queue(msg -> {
