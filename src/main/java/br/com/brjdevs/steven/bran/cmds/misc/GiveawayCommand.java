...
import br.com.brjdevs.steven.bran.core.utils.DateUtils;
	...
	private static ICommand giveaway() {
		return new TreeCommandBuilder(Category.MISCELLANEOUS)
				...
				.addSubCommand(new CommandBuilder(Category.MISCELLANEOUS)
						...
						.setAction((event) -> {
							...
							if (!expiresIn.isPresent()) {
								...
								e = DateUtils.getTime(((String) expiresIn.get()), TimeUnit.MILLISECONDS);
								if (e > 10800000) {
				...
				.addSubCommand(new CommandBuilder(Category.INFORMATIVE)
						...
						.setAction((event) -> {
							BotContainer container = event.getBotContainer();
							...
							Member creator = giveaway.getCreator(container);
							embedBuilder.setFooter("Giveaway created by " + Util.getUser(creator == null ? null : creator.getUser()), creator == null ? null : giveaway.getCreator(container).getUser().getEffectiveAvatarUrl());
							String desc = "This giveaway is available for " + (giveaway.isPublic() ? "everyone" : "members with role `" + giveaway.getRole(container).getName() + "`") + ".\n\n";
							String participating = giveaway.getParticipants(container).stream().map(member -> Util.getUser(member.getUser())).collect(Collectors.joining("\n"));
				...
				.addSubCommand(new CommandBuilder(Category.MISCELLANEOUS)
						...
						.setAction((event) -> {
							...
							Role role = giveaway.getRole(event.getBotContainer());
				...
				.addSubCommand(new CommandBuilder(Category.MISCELLANEOUS)
						...
						.setAction((event) -> {
							...
							embedBuilder.setColor(Color.decode("#43474B"));
							embedBuilder.setAuthor("Information on the Giveaway for Guild " + event.getGuild().getName(), null, event.getGuild().getIconUrl());
							embedBuilder.setFooter("Giveaway created by " + Util.getUser(giveaway.getCreator(container).getUser()), giveaway.getCreator(container).getUser().getEffectiveAvatarUrl());
							String desc = "This giveaway was available for " + (giveaway.isPublic() ? "everyone" : "members with role `" + giveaway.getRole(container).getName()) + ".\n";
							String participating = giveaway.getParticipants(container).stream().map(member -> Util.getUser(member.getUser())).collect(Collectors.joining("\n"));
							if (participating.length() > EmbedBuilder.TEXT_MAX_LENGTH - desc.length())
								participating = "The list was too long so I uploaded it to Hastebin: " + Hastebin.post(participating);
							...
							for (int i = 0; i < giveaway.getMaxWinners() && !p.isEmpty(); i++) {
								Long l = Util.random(p);
								p.remove(l);
								Member m = event.getGuild().getMemberById(String.valueOf(l));
								if (m == null) continue;
								winners.add(m);
							}
							desc += participating + "\n\nThere was " + giveaway.getTotalParticipants() + " users participating on this Giveaway!\n\nAnd the " + (giveaway.getMaxWinners() > 1 ? "winners are" : "winner is") + "... " + winners.stream().map(m -> Util.getUser(m.getUser())).collect(Collectors.joining("\n"));
							embedBuilder.setDescription(desc);
							event.sendMessage(embedBuilder.build()).queue();
							event.sendMessage("Congratulations, " + (winners.stream().map(m -> m.getUser().getAsMention()).collect(Collectors.joining(", "))) + "! You won this Giveaway, contact " + Util.getUser(giveaway.getCreator(container).getUser()) + " to receive your prize(s)! :smile:").queue();
							winners.forEach(member -> member.getUser().openPrivateChannel().queue(channel -> channel.sendMessage("Hey there! Congratulations! You were one of the winners in a Giveaway running in " + giveaway.getGuild(container).getName() + ", contact " + Util.getUser(giveaway.getCreator(container).getUser()) + " to receive your prize(s)!").queue()));
							event.getDiscordGuild().setGiveaway(null);
						})
						.build())
				.build();
