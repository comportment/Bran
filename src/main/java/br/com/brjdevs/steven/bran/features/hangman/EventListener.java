...
public class EventListener implements IEventListener {
	...
	public void onEvent(HangManEvent hangManEvent) {
		if (hangManEvent instanceof GuessEvent) {
			...
			event.getGame().getChannel().sendMessage(event.getGame().createEmbed().setDescription(String.format(s, Util.getUser(event.getProfile().getUser(event.getJDA())))).build()).queue();
