		...
		CommandManager.addCommand(new TreeCommandBuilder(Category.FUN)
				...
				.addCommand(new CommandBuilder(Category.FUN)
						...
						.setAction((event, rawArgs) -> {
							...
							if (event.getGuild().getCustomCommands().hasCustomCommand(cmdName)) {
								event.sendMessage(Quote.getQuote(Quote.FAIL) + "This Guild already has a command named **" + cmdName + "**, if you want to add answers to the command use `" + event.getPrefix() + "cmds addanswer " + cmdName + " [answer]`.").queue();
								return;
							}
				...
				.addCommand(new CommandBuilder(Category.FUN)
						...
						.setAction((event, rawArgs) -> {
							...
							if (!event.getGuild().getCustomCommands().hasCustomCommand(cmdName)) {
								event.sendMessage(Quote.getQuote(Quote.FAIL) + "This Guild does not have a Custom Command named **" + cmdName + "**, if you want to create one use `" + event.getPrefix() + "cmds create " + cmdName + (newAnswer.isEmpty() ? " [command Answer]" : " " + newAnswer) + "`.").queue();
								return;
							}
							...
							if (command.getAnswers().contains(newAnswer)) {
								event.sendMessage(Quote.getQuote(Quote.FAIL) + "This Command already has this answer!").queue();
								return;
							}
							command.getAnswers().add(newAnswer);
							event.sendMessage(Quote.getQuote(Quote.SUCCESS) + "Added a new answer for **" + cmdName + "**! This Command currently has " + command.getAnswers().size() + " answers.").queue();
						})
						.build())
				.addCommand(new CommandBuilder(Category.FUN)
						...
						.setAction((event, rawArgs) -> {
							...
							if (command == null) {
								event.sendMessage(Quote.getQuote(Quote.FAIL) + "This Guild does not have a Custom Command named **" + cmdName + "**, if you want to delete a specific answer from a command use `" + event.getPrefix() + "cmds rmanswer [command Name] [answer Index]` and if you want to delete a command use `" + event.getPrefix() + "cmds delete [command Name]`").queue();
								return;
							}
							if (!command.getCreatorId().equals(event.getAuthor().getId())
									&& !event.getMember().hasPermission(Permissions.GUILD_MOD, event.getJDA())) {
							...
							event.sendMessage(Quote.getQuote(Quote.SUCCESS) + "Removed answer index `" + index + "` from " + cmdName + ".\n").queue();
						})
						.build())
				.addCommand(new CommandBuilder(Category.FUN)
						...
						.setAction((event, rawArgs) -> {
							...
							CustomCommand command = event.getGuild()
									.getCustomCommands().getCustomCommand(cmdName);
							if (command == null) {
								event.sendMessage(Quote.getQuote(Quote.FAIL) + "This Guild does not have a Custom Command named **" + cmdName + "**.").queue();
								return;
							}
							if (!command.getCreatorId().equals(event.getAuthor().getId())
									&& !event.getMember().hasPermission(Permissions.GUILD_MOD, event.getJDA())) {
							...
							event.sendMessage(Quote.getQuote(Quote.SUCCESS) + "Deleted Custom Command `" + cmdName + "`.").queue();
						})
						.build())
						...
						.addCommand(new CommandBuilder(Category.GUILD_ADMINISTRATOR)
								...
								.setAction((event) -> {
									event.getGuild().getCustomCommands().isEnabled
											= !event.getGuild().getCustomCommands().isEnabled;
									event.sendMessage(Quote.getQuote(Quote.SUCCESS) +
											(event.getGuild().getCustomCommands().isEnabled ?
