	...
	private static ICommand permission() {
        return new TreeCommandBuilder(Category.GUILD_ADMINISTRATOR)
                ...
                .addSubCommand(new CommandBuilder(Category.BOT_ADMINISTRATOR)
                        .setAliases("tutorial", "how2use")
                        .setName("Permissions Tutorial Command")
                        .setDescription("Gives you a small help on how to use the permission command!")
                        .setAction((event) -> {
                            EmbedBuilder embedBuilder = new EmbedBuilder();
                            embedBuilder.setAuthor("Tutorial: Permissions", null, "https://i.imgur.com/B41jDnB.png");
                            embedBuilder.appendDescription("First thing let's make something clear: Bran does **not** interact with Discord permissions.\n\n");
                            embedBuilder.appendDescription("```md\n");
                            embedBuilder.appendDescription("1. How do I give someone permissions? R: You have to use the '.perms set' command. *e.g. .perms set +DJ @" + Utils.getUser(event.getAuthor()) + "*\n\n");
                            embedBuilder.appendDescription("1.     1. I can't give a permission myself! R: Read the message he replies.\n\n");
                            embedBuilder.appendDescription("1.     2. It says I don't have enough permissions! R: In order to assign permissions you need PERMSYS_GM plus the permission you want to assign.\n\n");
                            embedBuilder.appendDescription("2. How can I see all the available permissions? R: '.perms list'\n\n");
                            embedBuilder.appendDescription("3. How can I see my permissions? R: '.perms get' *e.g. .perms get @" + Utils.getUser(event.getAuthor()) + "*\n\n");
                            embedBuilder.appendDescription("4. It says I don't have enough permissions when I use '.perms set'! R: In order to assign permissions you need PERMSYS_GM plus the permission you want to assign.\n");
