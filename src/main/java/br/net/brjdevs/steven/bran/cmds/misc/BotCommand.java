...
import br.net.brjdevs.steven.bran.ExitCodes;
...
import static br.net.brjdevs.steven.bran.core.managers.Permissions.BOT_ADMIN;
		...
		return new TreeCommandBuilder(Category.MISCELLANEOUS)
				...
				.addSubCommand(new TreeCommandBuilder(Category.INFORMATIVE)
						...
						.addSubCommand(new CommandBuilder(Category.INFORMATIVE)
								...
								.setAction((event) -> event.sendMessage(Bran.getInstance().getSession().toEmbedCmds()).queue())
								.build())
						.addSubCommand(new CommandBuilder(Category.INFORMATIVE)
								.setAliases("guilds")
				...
				.addSubCommand(new CommandBuilder(Category.INFORMATIVE)
						...
						.setAction((event) -> {
                            ...
                            event.getChannel().sendTyping().queue(success -> {
                                ...
                                event.getChannel().sendMessage("\uD83C\uDFD3 Pong: `" + ping + "ms` (WS: `" + event.getJDA().getPing() + "ms`)").queue();
                            });
                        })
                        .build())
						...
						.addSubCommand(new CommandBuilder(Category.BOT_ADMINISTRATOR)
								...
								.setAction((event) -> {
									try {
                                        ...
                                        Bran.getInstance().getDataManager().getConfig().update();
